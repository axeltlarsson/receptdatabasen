...
class Image
{
  ...
  public function __construct($image, $caption)
  {
    ...
    if (strpos($image, $_ENV["IMAGE_UPLOAD_PATH"]) !== false) {
      $this->_path = $image;
      return;
    }
    ...
    $this->_path = $_ENV["IMAGE_UPLOAD_PATH"] . "/" . uniqid() . $extension;
  ...
  function getBase64()
  {
    ...
    // Själva bilden
    $image = @fread($handle, filesize($this->_path));
    @fclose($handle);

    // Filändelse från filsökvägen
    $ext = substr(strrchr($this->_path,'.'),1);
