$(function(){ 
    ...
    $("#changeNbrOfPersons").on('click', function() {
     ...
     $("#resetNbrOfPersons").on('click', function() {
         ...
         $('.ingredient').each(function() {
			var origIngredientString = $(this).children('li').attr('data-orig-ingredient-string');
			$(this).children('li').text(origIngredientString);
		});
    });
	...
	function changeQuantityOfIngredients() {
		...
		$('.ingredient li').each(function() {
			var origIngredientString = $(this).attr('data-orig-ingredient-string');
			var newIngredientString = origIngredientString;
			
			/*------------------------------------------------
				Ta hand om "½" i olika former
			-------------------------------------------------*/
			// Om ingrediens innehåller "X ½" eller "½"
			var halfRegex = /^½|\d+\s½/.exec(origIngredientString); // matchar först "½" i början sedan "X ½" där X är ett nummer
			if (halfRegex) { // innehåller ngn form av "½"
				// Lägg ihop "X ½" till "X+0.5"
				var nbr = /\d+/.exec(halfRegex); // matchar "X"
				if (nbr) {
					nbr = parseInt(nbr) + 0.5;
					newIngredientString = origIngredientString.replace(halfRegex, nbr);
				} else {
					newIngredientString = origIngredientString.replace('½', 0.5);
				} 
			...
			/*----------------------------------------------
				Omvandla kvanitet
			------------------------------------------------*/
			// Matchar newIngredientString mot regex - bearbetar talet i callbackfunktionen
			var regex = /\d+\.\d*|\d+\,\d*|\d+/g; // matchar tal
			newIngredientString = newIngredientString.replace(regex, function($0, $1) {
